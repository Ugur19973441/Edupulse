...
export default function TeacherMessages(){
  const [list, setList] = useState<Msg[]>([{ id:1, from:'Parent', text:'Merhaba hocam', ts:'10:00' }]);
  const [text, setText] = useState('');
  const endRef = useRef<HTMLDivElement>(null);

  useEffect(()=>{ endRef.current?.scrollIntoView({ behavior:'smooth' }); }, [list]);

  function send(){ if(!text.trim()) return; const id = list.length? Math.max(...list.map(m=>m.id))+1 : 1; setList([...list,{ id, from:'Teacher', text, ts: new Date().toLocaleTimeString() }]); setText(''); }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border rounded h-80 overflow-auto p-3 bg-slate-50">
        {list.map(m=> (
          <div key={m.id} className={`mb-2 flex ${m.from==='Teacher'?'justify-end':'justify-start'}`}>
            <div className={`px-3 py-2 rounded max-w-xs ${m.from==='Teacher'?'bg-brand text-white':'bg-white border'}`}>
